...
const ChallengeLinkModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  const protestFont = { fontFamily: "'Protest Revolution', sans-serif" };
      ...
      <div className="relative w-full max-w-lg bg-white border-4 border-black p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] rounded-sm window-popout">
        ...
        <div className="text-center mb-8">
          ...
          <p className="font-serif text-gray-600">
            Send this link to challenge a friend to the same lesson
          </p>
        </div>
        <div className="text-center mb-6">
          <p className="text-xs font-serif uppercase tracking-widest text-gray-400 mb-2">
            Room Code
          </p>
          <div
        ...
        <div className="flex gap-2 mb-6">
          <div className="flex-1 border-2 border-black p-3 font-serif text-sm truncate bg-gray-50">
            {challengeLink}
          </div>
          <button
            onClick={handleCopy}
            className={`px-4 py-2 border-2 border-black font-serif text-sm uppercase transition-all ${copied ? "bg-green-500 text-white border-green-500" : "bg-black text-white hover:bg-gray-800"}`}
          >
            {copied ? "✓ Copied" : "Copy"}
          </button>
        </div>
        <div className="border-2 border-dashed border-gray-300 p-4 text-center mb-6">
        ...
        <p className="text-center font-serif text-xs text-gray-400">
          * Multiplayer is in beta — both players need the link to join
        </p>
      </div>
    </div>
  );
...
const Results: React.FC<ResultsProps> = ({
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-[#f8f7f4] flex flex-col items-center justify-center p-4">
      ...
      {showChallengeModal && (
        <ChallengeLinkModal onClose={() => setShowChallengeModal(false)} />
      )}
